...
def training_process(batch_size: int, checkpoint_interval: int, clip_gradient_norm: int,
    ...
    total_dataset_size = 288  # 288 is the total number of training files in MuN
    one_dataset_len: int = total_dataset_size // len(dataset_training)
    train_dsets = []
    assert len(dataset_training) > 0, "No datasets were selected for training"
    ds: PianoRollAudioDataset
    for ds in dataset_training:
        # random.sample ensures that each value appears at least once in the list
        if len(ds) >= one_dataset_len:
            ds_indices = random.sample(range(0, len(ds)), one_dataset_len)
        else:
            ds_indices = []
            while len(ds_indices) + len(ds) < one_dataset_len:
                ds_indices.extend(range(0, len(ds)))
            ds_indices.extend(random.sample(range(0, len(ds)), one_dataset_len - len(ds_indices)))
