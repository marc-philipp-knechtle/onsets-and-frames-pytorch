...
training_configs_non_piano = {

}


training_configs_mixed = {
...
def config():
    ...
    training_size: int = 130 # max 130 because of non-piano size limit
...
def training_process(batch_size: int, checkpoint_interval: int, clip_gradient_norm: int,
    ...
    if train_on == 'piano':
        dataset_training: List[Dataset] = [
            ...
            ddef['PhA_train'](),
            ddef['CSD_train'](),
            ddef['RWC_non-piano_train'](),
            ddef['MuN_non-piano_train'](),
        ]
    ...
    total_training_samples = training_size
    one_dataset_testfiles_len: int = total_training_samples // len(dataset_training)

    train_dsets_subset_lens = {}
    train_dsets_lens = {}
    ds: PianoRollAudioDataset
    for ds in dataset_training:
        train_dsets_lens[str(ds)] = len(ds)
        train_dsets_subset_lens[str(ds)] = min(one_dataset_testfiles_len, len(ds))

    assigned_files = sum(indices for indices in train_dsets_subset_lens.values())
    remainder = total_training_samples - assigned_files

    while remainder > 0:
        for dset, length in train_dsets_subset_lens.items():
            if length < train_dsets_lens[dset]:
                train_dsets_subset_lens[dset] = length + 1
                remainder -= 1
                break

    for ds in dataset_training:
        allowed_len = train_dsets_subset_lens[str(ds)]
        ds_indices = random.sample(range(0, len(ds)), allowed_len)
...
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on, data_path, batch_size,
          ...
          leave_one_out, clip_gradient_norm, validation_length, validation_interval, clear_computed, training_size):
    ...
    try:
        training_process(batch_size, checkpoint_interval, clip_gradient_norm, device, iterations, learning_rate,
                         ...
                         writer, clear_computed, training_size)
    except Exception as e:
